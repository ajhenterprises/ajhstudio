...
export function churchPlan(ids: string[]) { return churchPlans.find(p => ids.includes(p.id)); }
...
export const entryServices: ProjectService[] = [
 ...churchPlans.map((p, i): ProjectService => ({id:p.id,name:`Church Website — ${p.name}`,shortDescription:"Professionally built church/ministry website, hosting, maintenance and reasonable routine content updates. Same core service on both plans. Month-to-month; cancel anytime.",category:"Websites",pricingType:"mixed",oneTimePrice:p.setup,monthlyPrice:p.monthly,projectType:"church_websites",priceNote:`$${p.setup} setup + $${p.monthly}/month`,active:true,displayOrder:i,icon:"church"})),
 {id:"website-content-refresh",name:"Website Content Refresh",shortDescription:"Refresh existing website content with clearer messaging, stronger calls-to-action, better readability, organization and content-level SEO. Content only; redesign and development require a separate quote.",category:"Content & Marketing",pricingType:"one-time",oneTimePrice:99,includedPages:5,additionalPagePrice:55,projectType:"content",active:true,displayOrder:2,icon:"pen",priceNote:"$99 for up to 5 pages; $55 per additional page, up to 10 total. Larger projects receive a custom quote."}
];
