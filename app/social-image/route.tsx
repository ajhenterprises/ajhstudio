...
export async function GET(request: Request) {
  ...
  return new ImageResponse(
    <div style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",padding:56,background:alternate?`linear-gradient(125deg, ${brand.navy}, #124469)` : brand.navy,color:"white",fontFamily:"Inter",position:"relative"}}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={`data:image/png;base64,${logoData.toString("base64")}`} width={360} height={69} alt="" />
      <div style={{display:"flex",marginTop:28,fontSize:18,color:brand.accent,textTransform:"uppercase",letterSpacing:3}}>{category}</div>
      <div style={{display:"flex",marginTop:20,fontSize:title.length>90?40:52,lineHeight:1.12,letterSpacing:-2,maxWidth:1050}}>{title}</div>
      <div style={{display:"flex",marginTop:20,fontSize:21,lineHeight:1.4,color:"#d9e5f0",maxWidth:1020}}>{description}</div>
